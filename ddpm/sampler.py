...
class SamplerGrandEnsemble(Sampler):
    def __init__(
        self,
        model: torch.nn.Module,
        config,
        ...
        Args:
            ...
            config: Configuration settings for sampling.
        """
        super().__init__(model=model, config=config, dataloader=dataloader, inversion_transforms=inversion_transforms)
        self.loss_func = loss_dict["L1Loss"]
        ...
        if self.config.sampling_mode == "simple":
            ...
            for batch_idx, batch in tqdm(enumerate(self.dataloader), total=len(self.dataloader), desc="Sampling ", unit="batch"):
                ...
                if not os.path.isfile(save_path):
                    ...
                    self.logger.info(f"Sampling already done for {filename}. Switching to next sample.")
