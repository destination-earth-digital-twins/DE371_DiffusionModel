...
class Patch_EDMLoss:
    """This class comes from the github https://github.com/Zhendong-Wang/Patch-Diffusion.git, associated to the paper 
    ...
    def pachify(self, images, patch_size, padding=None):
        ...
        if rank==0:
            print("patch size dans edm los avaut :", patch_size)
        ...
        if rank==0:
            print("dans patch edm loss :")
            print("h ici vaut :",h)
            print("w ici vaut, ",w)
            print("pareil pour th", th)
            print("pareil pour tw", tw)
        if w == tw and h == th:
    ...
    def __call__(self, net, images, patch_size, resolution, labels=None, augment_pipe=None):
        ...
        weight_value = weight.mean().item()
        if rank==0:    
            print("shape de weight", weight_value)
        y, augment_labels = augment_pipe(images) if augment_pipe is not None else (images, None)
        n = torch.randn_like(y) * sigma
        yn = y + n

        D_yn = net(yn, sigma, x_pos=images_pos, class_labels=labels, augment_labels=augment_labels) #regarder le net pour prendre en compte image pos et 
        #prendre en compte patch size dans le unet
        loss = ((D_yn - y) ** 2)
        if rank == 0:
            print('shape de la loss dans patch loss', loss.shape)
        loss = reduce(loss, 'b ... -> b', 'mean')
        if rank==0:
            print("loss après reduce 1,", loss)
        loss = loss * weight_value
        if rank==0:
            print("loss ici après le reduce", loss)
            print("loss quand on applique la moyenne ",loss.mean())
        return loss.mean()
